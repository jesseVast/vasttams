...
class TestWebhookEndpoint:
    ...
    def test_list_webhooks(self, api_available):
    ...
    def test_get_webhook_by_id(self, api_available):
        """Test GET /service/webhooks/{id} endpoint"""
        if not api_available:
            pytest.skip("API not available")
        
        # Try to get a non-existent webhook
        response = requests.get(f"{BASE_URL}/service/webhooks/non-existent-id")
