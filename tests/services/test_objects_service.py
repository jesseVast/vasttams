...
class TestObjectStorageService:
    ...
    async def test_get_object_with_referenced_flows(self):
        ...
        assert obj is not None
        assert obj.id == object_id
        # referenced_by_flows should be computed from segments via JOIN
        assert isinstance(obj.referenced_by_flows, list)
        assert len(obj.referenced_by_flows) == 2
        assert flow_id1 in obj.referenced_by_flows
        assert flow_id2 in obj.referenced_by_flows
    ...
    async def test_get_objects(self):
        """Test get_objects with JOIN query result"""
        ...
        mock_db = Mock()
        mock_db.get_qualified_table_name = lambda name: f"vast.schema.{name}"
        # Mock JOIN query result (get_objects uses execute_sql, not query)
        mock_db.execute_sql = Mock(return_value={
            'data': {
                'id': [object_id1, object_id1, object_id2],  # object_id1 appears twice (has 2 flows)
                'size': [1000000, 1000000, 2000000],
                'timerange': ['0:0_100:0', '0:0_100:0', '100:0_200:0'],
                'created': ['2024-01-01T00:00:00Z', '2024-01-01T00:00:00Z', '2024-01-02T00:00:00Z'],
                'flow_id': [flow_id, str(uuid.uuid4()), None],  # object_id2 has no flows
                'segment_created': ['2024-01-01T00:00:00Z', '2024-01-01T01:00:00Z', None]
            }
        })
        ...
        assert len(objects) == 2  # Should have 2 unique objects
    ...
    async def test_delete_object_instance(self):
        ...
        } if 'SELECT' in sql and 'FROM' in sql and 'objects' in sql else None)
        ...
        def query_side_effect(table_name):
            if table_name == "object_instances":
...
class TestObjectStorageServiceAdditional:
    ...
    async def test_get_object_timerange_empty_string(self):
        ...
        mock_db = Mock()
        mock_db.get_qualified_table_name = lambda name: f"vast.schema.{name}"
        # Mock JOIN query with timerange as empty string
        mock_db.execute_sql = Mock(return_value=create_mock_join_result(object_id, timerange=''))
    ...
    async def test_get_object_size_update_from_s3_404(self):
        ...
        # But test should still pass - object should be returned
    ...
    async def test_create_object_metadata_string(self):
        ...
        # Set metadata as internal attribute (metadata is not part of Object model)
        obj._internal_metadata = {"storage_path": "/path/to/object"}
    ...
    async def test_delete_object_instance_by_label(self):
        ...
        mock_query.where.return_value = mock_query
        mock_query.delete.return_value = mock_query
        # First call (select) returns instance data, second call (delete) succeeds
        call_count = 0
        ...
        service._get_storage_path_from_instance = Mock(return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_delete_s3_object_no_s3_client(self):
        """Test _delete_s3_object when S3 client is not available"""
        storage_path = "/path/to/object"
        
        service = ObjectStorageService(Mock(), None)  # No S3 client
        result = await service._delete_s3_object(storage_path)
        
        assert result is False
    
    @pytest.mark.asyncio
    async def test_delete_s3_object_with_root_path_stripping(self):
        """Test _delete_s3_object strips root_path from storage_path"""
        storage_path = "/root/path/to/object"
    ...
    @pytest.mark.asyncio
    async def test_delete_s3_object_with_delete_method(self):
        """Test _delete_s3_object uses delete method if delete_object not available"""
        storage_path = "/path/to/object"
        
        mock_s3 = Mock()
        # S3Client has 'delete' method but not 'delete_object'
        del mock_s3.delete_object
        ...
        assert result is True
        assert mock_s3.delete.called
    
    @pytest.mark.asyncio
    async def test_delete_s3_object_boto3_import_error(self):
        """Test _delete_s3_object handles boto3 ImportError"""
        storage_path = "/path/to/object"
