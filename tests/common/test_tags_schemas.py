...
from vasttamsserver.common.tags.schemas import get_tags_schema, get_tags_projections
...
class TestTagsSchema:
    ...
    def test_get_tags_schema_has_expected_fields(self):
        ...
        expected_fields = ["id", "entity_type", "entity_id", "tag_name", "tag_value", 
    ...
    def test_get_tags_projections_expected_columns(self):
        """Test that projections contain expected columns"""
        projections = get_tags_projections()
        all_columns = [col for proj in projections for col in proj]
        expected_columns = ["id", "entity_type", "entity_id", "tag_name", "created_at", "updated_at", "deleted_date"]
        for col in expected_columns:
            assert col in all_columns, f"Column {col} not found in any projection"
